document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    const validateField = function (field) {
      ...
      if (!errorMessage && field.type === 'email' && value.trim() !== '') {
        if (!validators.email(value)) {
          errorMessage = 'Please enter a valid email address.';
        }
      }

      if (!errorMessage && field.type === 'tel' && value.trim() !== '') {
        if (!validators.tel(value)) {
          errorMessage = 'Please enter a valid phone number.';
        }
      }

      if (!errorMessage && field.dataset.minlength) {
        if (!validators.minlength(value, field.dataset.minlength)) {
          errorMessage = fieldLabel + ' must be at least ' + field.dataset.minlength + ' characters.';
        }
      }
  ...
  const ANIMATION_SELECTORS = '.animate-on-scroll, .fade-in, .slide-up, [data-animate]';
  ...
  const injectAnimationStyles = function () {
    ...
    style.textContent = [
      ...
      '.animate-on-scroll.' + VISIBLE_CLASS + '{opacity:1 !important;transform:translateY(0) !important;}',
      '.fade-in:not(.' + ANIMATION_CLASS + '){opacity:0;transition:opacity 0.6s ease;}',
      '.fade-in.' + VISIBLE_CLASS + '{opacity:1 !important;}',
      '.slide-up:not(.' + ANIMATION_CLASS + '){opacity:0;transform:translateY(40px);transition:opacity 0.7s ease,transform 0.7s ease;}',
      '.slide-up.' + VISIBLE_CLASS + '{opacity:1 !important;transform:translateY(0) !important;}'
    ].join('');
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '2089a7d1-20f0-4fd3-9355-91c4c0172248',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '2089a7d1-20f0-4fd3-9355-91c4c0172248';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '2089a7d1-20f0-4fd3-9355-91c4c0172248';
